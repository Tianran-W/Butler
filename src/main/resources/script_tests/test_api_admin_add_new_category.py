...
def test_admin_add_new_category():
    log_section("测试场景: 管理员新建物资类别")
    session = get_authenticated_session(ADMIN_CREDENTIALS)
    if not session:
        print("❌ 因登录失败，终止测试。")
        return

    url = f"{BASE_URL}/admin/materialsNewCategories"
    payload = {"categoryName": f"Test-Category-{int(time.time())}"}
    ...
    try:
        ...
        if response.status_code == 201:
            print("✅ 测试通过: 服务器成功创建新类别并返回201 Created。")
        else:
