...
def test_admin_get_material_alerts():
    log_section("测试场景: 管理员获取库存预警列表")
    session = get_authenticated_session(ADMIN_CREDENTIALS)
    if not session:
        print("❌ 因登录失败，终止测试。")
        return

    url = f"{BASE_URL}/admin/materialAlerts"
...
if __name__ == '__main__':
    test_admin_get_material_alerts()