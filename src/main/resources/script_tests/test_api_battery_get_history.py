...
def test_get_battery_history():
    log_section(f"测试场景: 查询物资ID {MATERIAL_ID_FOR_HISTORY} 的电池历史状态")
    session = get_authenticated_session(USER_CREDENTIALS)
    if not session:
        print("❌ 因登录失败，终止测试。")
        return

    url = f"{BASE_URL}/batteryHistory/{MATERIAL_ID_FOR_HISTORY}"
