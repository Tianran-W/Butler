...
def test_submit_battery_status():
    log_section("测试场景: 提交电池状态")
    session = get_authenticated_session(USER_CREDENTIALS)
    if not session:
        print("❌ 因登录失败，终止测试。")
        return

    url = f"{BASE_URL}/batteryStatus"
    payload = {"materialId": MATERIAL_ID_FOR_BATTERY, "batteryLevel": 92, "batteryHealth": "良好"}
