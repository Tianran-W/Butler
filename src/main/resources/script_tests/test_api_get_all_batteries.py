...
# --- 测试函数 ---
def test_get_all_batteries():
    log_section("测试场景: 用户获取所有电池列表")
    ...
    try:
        ...
        if response.status_code == 200 and isinstance(response.json(), list):
            print("✅ 测试通过: 成功获取电池列表，且格式为JSON数组。")
        else:
            print(f"❌ 测试失败: 期望状态码为 200 且响应为列表，但实际状态码为 {response.status_code} 或格式不正确。")
    except requests.exceptions.RequestException as e:
        log_section("请求异常")
        print(f"❌ 请求过程中发生错误: {e}")
