...
def test_get_material_by_sn():
    log_section(f"测试场景: 通过SN码 '{SN_CODE_TO_QUERY}' 查询物资")
    session = get_authenticated_session(USER_CREDENTIALS)
    if not session:
        print("❌ 因登录失败，终止测试。")
        return

    url = f"{BASE_URL}/material/sn/{SN_CODE_TO_QUERY}"
    ...
    try:
        ...
        log_section("最终诊断")
        if response.status_code == 200 and response.json().get('snCode') == SN_CODE_TO_QUERY:
            print("✅ 测试通过: 服务器成功返回指定SN码的物资详情。")
        elif response.status_code == 404:
             print(f"✅ 测试通过 (预期场景): 服务器返回404，表示SN码 '{SN_CODE_TO_QUERY}' 不存在。")
        else:
            print(f"❌ 测试失败: 期望状态码为 200 或 404，但实际为 {response.status_code}。")
    except requests.exceptions.RequestException as e:
        log_section("请求异常")
        print(f"❌ 请求过程中发生错误: {e}")

if __name__ == '__main__':
    test_get_material_by_sn()