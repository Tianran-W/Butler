...
def test_material_scrap():
    log_section("测试场景: 提交报废申请")
    session = get_authenticated_session(USER_CREDENTIALS)
    if not session:
        print("❌ 因登录失败，终止测试。")
        return

    url = f"{BASE_URL}/material/scrap"
    payload = {"materialId": MATERIAL_ID_TO_SCRAP, "reason": "Automated test scrap"}
