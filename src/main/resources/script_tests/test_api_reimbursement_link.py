...
def test_reimbursement_link():
    log_section("测试场景: 关联报销信息")
    session = get_authenticated_session(USER_CREDENTIALS)
    if not session:
        print("❌ 因登录失败，终止测试。")
        return

    url = f"{BASE_URL}/reimbursement/link"
    payload = {"materialId": MATERIAL_ID_TO_LINK, "reimbursementId": f"BX-TEST-{int(time.time())}"}
