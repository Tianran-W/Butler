...
def test_user_add_new_borrow():
    log_section("测试场景: 用户新建借用记录")
    session, user_info = get_authenticated_session(USER_CREDENTIALS)
    if not session:
        print("❌ 因登录失败，终止测试。")
        return

    url = f"{BASE_URL}/addNewBorrow"
    payload = {
        "materialId": 2,
        "userId": user_info['userId'],
        "isExpensive": 0,
        "usageProject": "Automated Test Project",
        "approvalReason": "N/A"
    }
