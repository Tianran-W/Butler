...
def test_user_find_borrowing():
    log_section("测试场景: 通过用户ID搜索相关借用")
    session, user_info = get_authenticated_session(USER_CREDENTIALS)
    if not session:
        print("❌ 因登录失败，终止测试。")
        return

    url = f"{BASE_URL}/findBorrowingByUserId"
    payload = {"userId": user_info['userId']}
    ...
    try:
        ...
        if response.status_code == 200 and isinstance(response.json(), list):
            print("✅ 测试通过: 服务器成功返回该用户的借用列表。")
        else:
            print("❌ 测试失败: 状态码不为200或响应格式不是列表。")
    except requests.exceptions.RequestException as e:
        log_section("请求异常")
        print(f"❌ 请求过程中发生错误: {e}")

if __name__ == '__main__':
    test_user_find_borrowing()